...
parser.add_argument('--binary_save_directory', required=True, help='folder where train.obj and val.obj is saved')
...
def main():
    ...
    data_dir = args.binary_save_directory
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch + 1) % args.schedule == 0:
            model.update_lr()
    batch_cnt = 0
    for vbid, val_batch in tqdm(enumerate(val_dataloader)):
        model.infer_sample(val_batch, os.path.join(sample_dir, str(global_steps)), batch_cnt)
        print("Checkpoint: save checkpoint step %d" % global_steps)
